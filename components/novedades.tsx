...
const newsItems = [
  ...
  {
    id: 4,
    title: "Nuevos Materiales para Cercos en 2025",
    excerpt: "Descubre las innovaciones que llegarán al mercado este año.",
    date: "30 Junio, 2025",
    image: "/placeholder.svg?height=400&width=600",
  }
]
...
export default function NewsCarousel() {
  ...
  const prevSlide = () => {
    setCurrentIndex((prev) => (prev - 1 + (totalItems - itemsPerView + 1)) % (totalItems - itemsPerView + 1))
  }
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="relative group">
          ...
          <button 
            ...
            className={`absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/90 hover:bg-white text-gray-800 p-2 rounded-full shadow-lg transition-all ${currentIndex === 0 ? 'opacity-50 cursor-not-allowed' : 'opacity-0 group-hover:opacity-100'}`}
          ...
          <button 
            ...
            className={`absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/90 hover:bg-white text-gray-800 p-2 rounded-full shadow-lg transition-all ${currentIndex >= totalItems - itemsPerView ? 'opacity-50 cursor-not-allowed' : 'opacity-0 group-hover:opacity-100'}`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 transition-transform duration-300">
            {visibleItems.map((item) => (
              <div key={item.id} className="bg-white rounded-xl shadow-md overflow-hidden group hover:shadow-lg transition-all duration-300">
          ...
          <div className="flex justify-center gap-2 mt-8">
            {Array.from({ length: totalItems - itemsPerView + 1 }).map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all ${index === currentIndex ? 'bg-brand w-6' : 'bg-gray-300'}`}
  ...
  <div className="grid md:grid-cols-2 gap-8 items-center">
    ...
    {/* Columna derecha - Carrusel de 3/1 imágenes */}
    <div className="relative h-64 md:h-80 w-full rounded-lg overflow-hidden">
      {/* Versión desktop (3 imágenes) */}
      ...
      {/* Versión móvil (1 imagen central) */}
      <div className="md:hidden absolute inset-0">
        <Image
          src="/Productos/novedades-rectangulo.webp" // Puedes elegir la imagen más representativa
